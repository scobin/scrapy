...
class ScrapyBlogSpiderSpider(scrapy.Spider):
    ...
    allowed_domains = ['developers-jp.googleblog.com']
    start_urls = ['http://developers-jp.googleblog.com/']
    ...
    def parse(self, response):
        ...
        for post in response.css('.Blog .post'):
            ...
            yield Post(
                url=post.css('div.post a::attr(href)').extract_first().strip(),
                title=post.css('div.post a::text').extract_first().strip(),
                date=post.css('div.post div.post-header span.publishdate::text').extract_first().strip(),
            )
